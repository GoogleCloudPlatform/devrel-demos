...
# Artifact Registry
resource "google_artifact_registry_repository" "pattern_d_repo" {
  project       = var.project
  location      = var.region
  repository_id = "cloud-train-pattern-d"
  description   = "Cloud Train Demo: Pattern D"
  format        = "DOCKER"
  depends_on = [
    time_sleep.wait
  ]
...
resource "google_cloudbuild_trigger" "new_build" {
  ...
  build {
    ...
    step {
      ...
      args    = ["builds", "submit", "--tag", "${var.region}-docker.pkg.dev/${var.project}/cloud-train-pattern-d/image", "target/"]
      timeout = "120s"
    }
  }
...
resource "google_cloudbuild_trigger" "new_release" {
  ...
  build {
    ...
    step {
      name    = "gcr.io/cloud-builders/gcloud"
      args    = ["deploy", "releases", "create", "pattern-D-release", "--project=${var.project}", "--region=${var.region}", "--delivery-pipeline=${google_clouddeploy_delivery_pipeline.default.name}", "--images=${"${var.region}-docker.pkg.dev/${var.project}/cloud-train-pattern-d/image:default"}"]
      timeout = "120s"
    }
  }
}
