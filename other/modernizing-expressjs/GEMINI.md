...
| :--- | :--- | :--- | :--- | :--- |
| 2026-04-03 | `npm run db:seed` hanging/failing | Standalone scripts run with `tsx` do not automatically load `.env.local`, unlike `next dev`. | Manually load env vars (e.g., `node --env-file=.env.local`). | Always ensure environment variables are explicitly loaded for standalone scripts. |
