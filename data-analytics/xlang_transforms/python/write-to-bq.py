  ...
  def process(self, element):
    ...
    yield beam.Row(
        ...
        distance_between_service=float(
          line_array[15] if line_array[15] != '' else 0),
...
def run(input_files, transform_jar, table_name, pipeline_args):
  ...
  write_to_bigquery = JavaExternalTransform(
      'com.google.beam.workshop.WriteToBigQuery',
      classpath=[transform_jar]
  ).writeTo(table_name).usingStorageApiAtLeastOnceMethod()
...
if __name__ == '__main__':
  ...
  run(known_args.input_files, known_args.transform_jar, known_args.table_name,
      pipeline_args)
