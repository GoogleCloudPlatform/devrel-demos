...
def main():
    console.print(Panel(
        Text.assemble(
            ("⚠️ Cloud Resources Teardown Plan Preview\n\n", "bold red"),
            ("This utility will surgically delete all provisioned GCP cloud resources ", "white"),
            ("including BigQuery tables, models, datasets, and IAM bindings. Local processes are NOT touched.\n", "white"),
            (f"Target Project: {PROJECT_ID}  |  Dataset to Drop: {DATASET_ID}", "bold yellow")
        ),
        border_style="red",
        title="[bold]CLOUD RESOURCES TEARDOWN PLAN[/bold]"
    ))
    ...
    bq_table = Table(title="BigQuery Assets & Schema", show_header=True, header_style="bold blue")
    ...
    conn_table = Table(title="Cloud Resource Connections & IAM Delegation", show_header=True, header_style="bold green")
    ...
    console.print("")
    confirm = Confirm.ask("[bold red]Are you sure you want to execute this GCP resource teardown?[/bold red]")
    if not confirm:
        console.print("[yellow]Teardown execution cancelled.[/yellow]")
        return

    console.print("\n[bold red]⚡ Executing Cloud Resources Teardown...[/bold red]")
    ...
    console.print(Panel(
        Text("✨ GCP assets successfully dropped! ✨", style="bold green", justify="center"),
        border_style="green",
        padding=(1, 2)
    ))
