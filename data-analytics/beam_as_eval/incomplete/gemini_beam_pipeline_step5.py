...
def run(argv=None):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        accuracy_predictions = (
            accuracy_inputs
            | 'VerifyAccuracy' >> RunInference(accuracy_model_handler)
        )

        # 4. Window the results and parse them into a final format
        final_results = (
            accuracy_predictions
            | 'AddTimestampBackForWindowing' >> beam.Map(
                lambda pr: beam.window.TimestampedValue(
                    pr, json.loads(json.loads(pr.example)['original_data_json'])['timestamp']
            ))
            | 'WindowIntoFixedWindows' >> beam.WindowInto(beam.window.FixedWindows(60))
            | 'ParseResults' >> beam.Map(
        ...
        good_bad_counts = (
            ...
            | 'AssignGoodBadKey' >> beam.Map(
                lambda element: ('good' if element[2] >= 0.7 else 'bad', (1, (element[0].get('prompt', ''), element[0].get('text', ''))))
        ...
        good_bad_counts | 'LogBadCounts' >> beam.Map(
            lambda count_tuple, window=beam.DoFn.WindowParam: logging.info(f"Window: {window}, Bad Counts: {count_tuple[1]}, Bad Prompts: {count_tuple[2]}")
        )
