...
for project in "${projects[@]}"; do
  ...
  printf "\rAuditing keys: %d/%d projects checked (Checking: %s)...\e[K" "$current" "$total_projects" "$project" >&2
  current=$((current + 1))
  
  # Fetch keys in JSON format
  keys_json=$(gcloud services api-keys list --project="$project" --format=json 2>/dev/null)
  
  # Verify keys_json is non-empty and contains valid JSON
  if [ -n "$keys_json" ] && echo "$keys_json" | jq -e . >/dev/null 2>&1; then
    # Parse and filter the valid JSON array
    echo "$keys_json" | jq --arg proj "$project" -c '
      .[] | 
      # Ensure the element is a valid API key object (must have a uid)
      # This filters out empty strings "", empty objects {}, and bad metadata
      select(.uid != null) |
      
      # Filter for keys without restrictions
      select(
