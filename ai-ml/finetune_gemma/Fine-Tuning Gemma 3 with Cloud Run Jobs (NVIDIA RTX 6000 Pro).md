...
Building state-of-the-art vision-language applications requires a powerful foundation. With the release of **[Gemma 3](https://huggingface.co/google/gemma-3-4b-it)**, Google's latest family of open multimodal models, and the introduction of **[NVIDIA RTX PRO 6000 Blackwell Server Edition GPUs](https://cloud.google.com/blog/products/serverless/cloud-run-supports-nvidia-rtx-6000-pro-gpus-for-ai-workloads)** on Cloud Run, developers can now fine-tune massive multimodal models in a fully managed, serverless environment.

### The Foundation: Gemma 3 4B
Gemma 3 4B is a native multimodal model designed for high-performance reasoning across images, text, and (optionally) audio. Unlike earlier VLMs that "bottlenecked" through a separate vision encoder, Gemma 3's architecture is built from the ground up to understand visual spatial relationships and generate nuanced human-like narratives.

### The Power: NVIDIA RTX 6000 Pro Blackwell GPUs
Cloud Run recently added support for the **NVIDIA RTX 6000 Pro**, which is built on the cutting-edge **Blackwell architecture**. These GPUs provide the massive VRAM (48GB) and compute throughput necessary to run Parameter-Efficient Fine-Tuning (PEFT) on 4B+ parameter models without the complexity of managing multi-node clusters.

### The Data: Deep Semantic Captions with DALL-E 3
To specialize Gemma 3 for **Deep Image Captioning**, we use the **[ProGamerGov Synthetic DALL-E 3](https://huggingface.co/datasets/ProGamerGov/synthetic-dataset-1m-dalle3-high-quality-captions)** dataset. This dataset is unique because it combines 1 million high-resolution AI-generated images with exceptionally dense, high-quality captions generated by CogVLM. By fine-tuning on this data, we teach Gemma 3 to move beyond simple labels and instead provide rich, descriptive narratives.

### The Orchestrator: Cloud Run Jobs
[Cloud Run jobs](https://cloud.google.com/run/docs/create-jobs) is the ideal orchestrator for this workflow. It allows you to package your entire training pipeline—complete with **uv** for lightning-fast dependency management—and execute it as a containerized batch job. You only pay for the execution time, making serverless fine-tuning both cost-effective and remarkably simple to deploy.

In this guide, we walk through the process of fine-tuning the Gemma 3 4B model for **Deep Image Captioning** at scale.
...
> [!TIP]
> **If you skipped Step 2**, notice we changed the `--model-id` in the `--args` to `google/gemma-3-4b-it` instead of a local path. This tells the script to pull directly from Hugging Face.
...
Now that you've fine-tuned Gemma 3, you're ready to deploy it for production inference. To simplify this process, we recommend using **[`cr-infer`](https://github.com/oded996/cr-infer)**. 

`cr-infer` is a powerful CLI tool specifically designed to automate the deployment of AI workloads on Cloud Run with GPUs. It handles:
*   **Automated Model Management**: Easily moves your fine-tuned weights between GCS and Hugging Face.
*   **Optimized Deployments**: Automatically configures Cloud Run services with the correct GPU types, volume mounts, and networking.
*   **Interactive Testing**: Includes a built-in streaming chat interface to test your model immediately after deployment.

Whether you're building a prototype or a high-scale application, `cr-infer` makes the "last mile" of deployment remarkably simple.
