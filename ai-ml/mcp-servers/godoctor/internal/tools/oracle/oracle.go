...
func executeTool(ctx context.Context, name string, args map[string]interface{}) (*mcp.CallToolResult, error) {
	...
	switch name {
	case "list_files":
		var p list_files.Params
		json.Unmarshal(jsonBytes, &p)
		res, _, err := list_files.Handler(ctx, nil, p)
		return res, err
	case "read_docs":
		var p read_docs.Params
		json.Unmarshal(jsonBytes, &p)
		res, _, err := read_docs.Handler(ctx, nil, p)
		return res, err
	case "inspect_symbol":
		var p inspect_symbol.Params
		json.Unmarshal(jsonBytes, &p)
		res, _, err := inspect_symbol.Handler(ctx, nil, p)
		return res, err
	case "code_outline":
		var p code_outline.Params
		json.Unmarshal(jsonBytes, &p)
		res, _, err := code_outline.Handler(ctx, nil, p)
		return res, err
	case "go_test":
		var p go_test.Params
		json.Unmarshal(jsonBytes, &p)
		res, _, err := go_test.Handler(ctx, nil, p)
		return res, err
	}
