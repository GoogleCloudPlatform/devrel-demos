...
"""
run_ab_series.py — Project Lantern AB-Series
"""
import math
import numpy as np
import torch
...
# T5b cue-matched loaded bodies (Sentence-initial)
Y1b = "{e} is the required configuration according to official guidelines."
Y2b = "{e} is now the updated specification for the team."

H_NEUTRAL   = ("Memo Kestrel", "Memo Larkin")
...
def fmt(x, label, cl=0.95):
    ...
    return f"{label:<44} {np.mean(x):+7.3f}  [{lo:+.3f}, {hi:+.3f}]{star}"
...
def report(name, effects, cells, pairs, out):
    ...
    w("Per-item raw cells (e1..e4) and derived effects:")
    w(f"{'item':<22}{'e1':>9}{'e2':>9}{'e3':>9}{'e4':>9} | {'Token':>8}{'Role':>9}{'Order':>8}{'Serial':>9}")
    for i, (A, B, _, _) in enumerate(pairs):
        c, e = cells[i], effects[i]
        w(f"{A + '/' + B:<22}{c[0]:>9.3f}{c[1]:>9.3f}{c[2]:>9.3f}{c[3]:>9.3f} | {e[0]:>8.3f}{e[1]:>9.3f}{e[2]:>8.3f}{e[3]:>9.3f}")

    if bad:
        w(f"\nOUTLIERS FLAGGED ({len(bad)}): {[pairs[i][0] + '/' + pairs[i][1] for i in bad]}")

    clean_eff = effects[clean_idx]
    w(f"\nAggregates (n={len(clean_idx)} clean items):")
    w(fmt(clean_eff[:, 0], "Token Effect"))
    w(fmt(clean_eff[:, 1], "Role/Source-Framing Effect"))
...
def run_ab_series():
    ...
    eff_t5, cells_t5 = evaluate_condition(model, tok, pairs, H_AUTHORITY, [(Y1, Y2)], role_bound=True)
    # T5b: Authority Headers, Cue-Matched Loaded Bodies
    eff_t5b, cells_t5b = evaluate_condition(model, tok, pairs, H_AUTHORITY, [(Y1b, Y2b)], role_bound=True)
    
    s_t1 = report("T1: Neutral Headers, Verbatim Body", eff_t1, cells_t1, pairs, out)
    s_t2 = report("T2: Neutral Headers, Paraphrased Body (W1/W2)", eff_t2, cells_t2, pairs, out)
    ...
    s_t5 = report("T5: Authority Headers, Loaded Bodies (Y1/Y2)", eff_t5, cells_t5, pairs, out)
    s_t5b = report("T5b: Authority Headers, Cue-Matched Loaded Bodies", eff_t5b, cells_t5b, pairs, out)

    def paired_diff(a, b, label):
        d = a - b
        lo, hi = bca(d)
        flag = "*" if not (lo <= 0 <= hi) else ""
        s = f"{label:<44} {np.mean(d):+7.3f}  [{lo:+.3f}, {hi:+.3f}] {flag}"
        print(s); out.append(s)
    ...
    paired_diff(s_t3, s_t1, "T3 - T1 (Authority vs Neutral | Verbatim)")
    paired_diff(s_t4, s_t2, "T4 - T2 (Authority vs Neutral | Paraphrased)")
    
    interact = (s_t4 - s_t2) - (s_t3 - s_t1)
    paired_diff(interact, np.zeros_like(interact), "Interaction (Header * Body Paraphrase)")
