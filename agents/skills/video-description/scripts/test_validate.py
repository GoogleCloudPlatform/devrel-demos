...
#AI #Agents #Tech #Coding #DeepDive"""
    ...
    elif "vacation video" in query_lower:
...
def check_semantic_expectations(output, expected_behaviors):
    ...
    for behavior in expected_behaviors:
        ...
        if "first-person" in behavior_lower or "i " in behavior_lower:
             if " i " not in output_lower and " my " not in output_lower:
        ...
        if "hashtag" in behavior_lower:
            ...
            if "exactly 5" in behavior_lower and count != 5:
                 failures.append(f"Hashtag count mismatch (found {count}): '{behavior}'")
            elif "fewer than 5" in behavior_lower and count >= 5:
                 failures.append(f"Hashtag count mismatch (found {count}): '{behavior}'")

    return failures

@pytest.mark.parametrize("test_case", load_evaluations())
def test_video_description_generation(test_case):
    """
