    ...
    return (
        <tr className="hover:bg-white/[0.02] transition-colors text-body">
            ...
            <td className="px-6 py-4">
                <Link href={`/experiments/view?id=${exp.id}`} className="font-bold text-[#f4f4f5] hover:text-[#6366f1] transition-colors truncate block max-w-xs">
                    {exp.name || "—"}
                </Link>
            ...
            <td className="px-6 py-4">
                <span className={`inline-flex items-center px-2 py-0.5 rounded-[3px] font-mono font-bold uppercase tracking-wider ${exp.status.toUpperCase() === 'COMPLETED' ? 'bg-green-950/30 text-green-400 border border-green-900/50' :
