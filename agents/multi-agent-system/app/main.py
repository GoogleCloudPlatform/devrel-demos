...
async def chat_stream(request: SimpleChatRequest):
    ...
    async def event_generator():
        ...
        yield json.dumps({"type": "progress", "text": "🌐 Discovering agents..."}) + "\n"
        if not agent_name:
            agent_name = (await list_agents(agent_server_url))[0] # type: ignore

        session = None
        if request.session_id:
            yield json.dumps({"type": "progress", "text": "🔑 Resuming session..."}) + "\n"
        ...
        if session is None:
            yield json.dumps({"type": "progress", "text": "🔑 Initializing new session..."}) + "\n"
        ...
        yield json.dumps({"type": "progress", "text": "🚀 Assembling your AI squad..."}) + "\n"
